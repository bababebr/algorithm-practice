...
Так как на вход подается n команд, то временная сложность будет O(n) * O(1) = O(n)

-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
Так как мы сразу знаем размер дека, то памяти выделяется O(m), где m - размер дека, который задается из ввода.
Так же память занимают переменные max_n, head, tail, size, но этим можно принебречь.
В так случае сложность равно O(m).
*/
